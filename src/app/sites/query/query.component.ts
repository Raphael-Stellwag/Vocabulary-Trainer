...
export class SiteQueryComponent {
    private class: string;
    private unit: string;
    private unitSetted = true;
    public vocsToQuery: IVocabulary[] = [];
    private correct = 0;
    private failures = 0;
    public index = 0;
    ...
    constructor(public vocService: VocabularyService, public router: Router, public route: ActivatedRoute, public dialog: MatDialog) {
        ...
        if (this.unit === undefined || this.unit === null || this.unit === '') {
            ...
            vocService.getVocsFromOneClas(this.class).then((result: IVocabulary[]) => {
                this.vocsToQuery = result;
            });
        } else {
            vocService.getVocsFromOneUnit(this.class, this.unit).then((result: IVocabulary[]) => {
                this.vocsToQuery = result;
            });
        }
    }
